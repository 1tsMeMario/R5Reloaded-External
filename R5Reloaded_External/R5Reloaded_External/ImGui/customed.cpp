...
bool ImGui::Toggle(const char* label, bool* v, bool a)
{
	...
	if (a)
		window->DrawList->AddCircleFilled(ImVec2(pos.x + radius + t * (width - radius * 2.0f), pos.y + radius), radius - 1.5f, ImColor(0.f, 1.f, 0.f, 1.f), 36);
	else
		window->DrawList->AddCircleFilled(ImVec2(pos.x + radius + t * (width - radius * 2.0f), pos.y + radius), radius - 1.5f, ImColor(1.f, 1.f, 1.f, 1.f), 36);
